...
export default {
  ...
  data() {
    return {
      ...
      searchParams: {
        ...
        pageNo: 1, // 第几页
        pageSize: 10, // 一页有多少个商品
        props: [], // 属性值
        trademark: "", // 品牌
      },
    };
  },
  // 当组件挂载完毕之前执行一次|可以在发请求之前将带有参数进行修改
  beforeMount() {
    // 发请求之前，把接口需要传递的参数，进行整理(在给服务器发请求之前，把参数整理好，服务器就会返回查询的数据)
    Object.assign(this.searchParams, this.$route.query, this.$route.params);
  },
  mounted() {
    // 在发送请求之前带给服务器(searchParams参数发生变化有数值带给服务器)
    this.getData();
  },
  computed: {
  ...
  methods: {
    ...
    // 把请求封装成函数，需要调用的时候调用即可
    getData() {
      this.$store.dispatch("searchList", this.searchParams);
    },
    // 删除分类的名字
    removeCategoryName() {
      // 把带给服务器的参数置空 参数都是可有可无的，定义为undefined就可以不发送
      ...
      // 再次发送请求
      this.getData();
      // 地址栏也需要修改，进行路由跳转(本意删除query,若有params保留)
    ...
    // 排序操作 flag是新的数据，用于区分综合、价格
    changeOrder(flag) {
      let newSearchOrder = this.searchParams.order;
      // 这里获取开始状态  将order拆为两个字段orderFlag(1:2)、orderSort(asc:desc)
      ...
      if (orderFlag !== flag) {
        ...
        // 多次点击的是同一个按钮
        newSearchOrder = `${flag}:${orderSort === "desc" ? "asc" : "desc"}`;
      }
      // 需要给order重新赋值

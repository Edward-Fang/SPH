          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              >
                ...
                <dd
                  changepirce="0"
                  :class="{ active: valueName.isChecked == 1 }"
                  v-for="valueName in saleAttr.spuSaleAttrValueList"
                  :key="valueName.id"
                  @click="
                    changeActive(valueName, saleAttr.spuSaleAttrValueList)
                  "
                >
                  {{ valueName.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
...
export default {
  ...
  methods: {
    ...
    // 发请求之前把购买的产品信息通过请求的形式通知服务器，服务器进行相应的存储
    async addShopCar() {
      // 发请求  将产品加入数据库
      // 存储成功 进行路由跳转传递参数    失败 给用户提示
      try {
